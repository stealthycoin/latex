...
{
  fprintf ("Usage: %s [options] key\n");
  fprintf ("\t-o <output file (default=stdout)\n");
}
...
main(int argc, char **argv)
{
  ...
  /* Not secure, but so what? */
  srand (time((void *)0) ^ getpid () ^ ((time((void *)0) & 0xffff) << 16));
  output = stdout;
  while ((ch = getopt (argc, argv, "m:o:i:")) != -1) {
    switch (ch) {
    case 'o':
      outfile = optarg;
  ...
  if (argc < 4)
  {
    fprintf (stderr, "Usage: %s <key> <mode> <output file>\n", argv[0]);
    return 1;
  }
  /*
   * Get the key from the password.  The key is specified as a string of
  ...
  while (!feof(stdin))
  {
	...
	plaintext[i] ^= ciphertext[i];
      ...
      /* CTR mode */
      rijndaelEncrypt (rk, nrounds, buf, ciphertext);
    }
